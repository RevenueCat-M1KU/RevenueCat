 ...
 */
export async function checkEntitlement(settings: Settings, user: string): Promise<Entitlement> {
  const { RC_SECRET_KEY: key, RC_PROJECT_ID: project, RC_ENTITLEMENT_ID: listen } = settings
  if (!key || !project || !listen) return 'unknown'
  const path = `/v2/projects/${encodeURIComponent(project)}/customers/${encodeURIComponent(user)}/active_entitlements`
  try {
