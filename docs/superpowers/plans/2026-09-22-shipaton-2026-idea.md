...
| 3     | Which ideas can ship in time?                          | Screen each candidate against the hard constraints, pass or fail, with the reason                      | About 12 survivors               |
...
constraint it fails. Keep about 12 survivors. If more pass, drop those in the
most crowded gallery clusters first. If fewer than eight pass, relax no
constraint; instead rescue failed ideas that one named change would fix.
