...
- **Where the spec lives.** The OpenAPI file RevenueCat's docs link,
  ...
  ([rc-openapi-docs]). RevenueCat's CLI vendors the full spec, and it is
  quoted here ([rc-spec]). Permission, rate limit, and the `404` are in the
  [RevenueCat and Expo notes][rc-v2-notes].
...
- **The 200 body.** `ListCustomerActiveEntitlements` requires `items`,
  ...
  this list can be accessed." ([rc-spec])
- **Each item.** `CustomerEntitlement` requires `object`, `entitlement_id`,
  ...
  `format: int64`, "in ms since epoch" ([rc-spec]).
...
- **Errors.** The operation lists `400`, `401`, `403`, `404`, `423`, `429`,
  ...
  `doc_url`, and `backoff_ms`, "The ms the client should wait before
  retrying the request. Only present for retryable errors." ([rc-spec])
...
- **401 and 403.** `Unauthorized` is `authentication_error`, "Invalid API
  ...
  access this resource"; both are `retryable: false` ([rc-spec]).
- **423.** `Locked` is `resource_locked_error`, "The resource is currently
  ...
  `backoff_ms: 1000` ([rc-spec]).
- **429.** `RateLimited` narrows `type` to `rate_limit_error` and sends
  `Retry-After`, "The number of seconds to wait before retrying a rate
  limited request", plus `RevenueCat-Rate-Limit-Current-Usage` and
  `RevenueCat-Rate-Limit-Current-Limit` ([rc-spec]). The docs' example body
  ([rc-api-v2]):

  ```json
  { "type": "rate_limit_error", "message": "Rate limit exceeded", "retryable": true, "backoff_ms": 1000 }
  ```
...
- **5xx.** `500` is "The RevenueCat server ran into an unexpected problem"
  ...
  both at `InternalError` ([rc-api-v2]; [rc-spec]).
- **Encoding.** "For URL params, such as the `app_user_id`, make sure you URL
  encode them before using them." ([rc-api-v2])
...
- **The package.** `@revenuecat/cli`, latest `0.1.3`, modified
  2026-09-18, binaries `rc` and `revenuecat`, Node `>=18` ([cli-npm]). Its
  `bin/rc.js` is "a thin launcher": "The CLI itself is a native Go binary",
  shipped as optional dependencies `@revenuecat/cli-<platform>-<arch>`, and
  "No JavaScript runs beyond this dispatch." ([cli-npm])
- **Logging in.** `rc auth login` offers "Browser login", which "Opens a
  browser window for OAuth authorization", or "API key", "a secret key from
  the dashboard", stored in `~/.config/revenuecat/<profile>.json` with
  mode 0600. "The API key can also be supplied via RC_API_KEY for CI use
  without storing anything on disk; while set, it overrides the stored
  login." ([cli-auth-src]) Credentials go to the OS keyring when available,
  and `RC_CONFIG_DIR` or `XDG_CONFIG_HOME` move the directory
  ([cli-config-src]).
- **Precedence.** `--api-key` wins over `$RC_API_KEY`, then the OAuth login,
  then the stored key; `--profile` picks the profile ([cli-readme]).
...
- **What it does.** "Creates a real RevenueCat Test Store transaction
  ...
  Product ID or store identifier." ([cli-customers-src])
- **What it needs.** A project, then `--app-id` (or `RC_APP_ID`),
  ...
  public SDK key". Under `--no-input`, `--yes` is "Required."
  ([cli-customers-src])
- **The request.** It posts `fetch_token`, `app_user_id`, and `product_id`
  ...
  `X-Client-Bundle-Id: com.revenuecat.cli` ([cli-sdk-src]). The token is
  `TEST_<ms>_<32 hex>` ([cli-customers-src]).
...
- **Exit codes.** 0 success, 1 error, 2 bad usage, 4 "Authentication /
  authorization", 5 not found, 6 rate limited ([cli-readme]).
...
- **The cursor.** `exec(query: string, ...bindings: any[]): SqlStorageCursor`.
  ...
  value is used for SQL billing" ([cf-do-sqlite]).
...
- **Tables.** The API page's example runs
  ...
  ([cf-do-sqlite]).
- Synthesis: the claim and the release each run in one `transactionSync()`
  with no `await` inside, and the Jev call happens between them, so a second
  line can arrive then; the claim alone decides free, duplicate, or paid.
...
- **The field allows null.** `expires_at` is `nullable: true` ([rc-spec]).
