...
- [ ] The speech controller takes an optional `gate: { beforeSpeak():
    Promise<void>; afterSpeech(): void }`, no-ops by default. `speakText()`
      awaits `gate.beforeSpeak()` after its stop, and returns if a newer
      `generation` took over. `onDone`, `onStopped`, the throw path, and
