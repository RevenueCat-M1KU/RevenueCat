...
1.  [Pull requests as a triage surface](#pull-requests-as-a-triage-surface)
1.  [When a skill says "publish to the issue tracker"](#when-a-skill-says-publish-to-the-issue-tracker)
1.  [When a skill says "fetch the relevant ticket"](#when-a-skill-says-fetch-the-relevant-ticket)
1.  [Wayfinding operations](#wayfinding-operations)
...
- **List issues**: run the command below, with `--label` and `--state`
  filters as needed.
...
- **Read a PR**: `gh pr view <number> --comments`, and `gh pr diff <number>`
  for the diff.
- **List external PRs for triage**: run the command below, then keep only an
  `authorAssociation` of `CONTRIBUTOR`, `FIRST_TIME_CONTRIBUTOR`, or `NONE`
  (drop `OWNER`, `MEMBER`, and `COLLABORATOR`).
...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue
  ...
  of the child body. Labels: `wayfinder:<type>`, where the type is
  `research`, `prototype`, `grilling`, or `task`. Once claimed, the ticket is
...
- **Frontier query**: list the map's open children
  (`gh issue list --state open`, scoped to the map's sub-issues or task list),
...
- **Resolve**: `gh issue comment <n> --body "<answer>"`, then
  `gh issue close <n>`, then append a context pointer (gist and link) to the
